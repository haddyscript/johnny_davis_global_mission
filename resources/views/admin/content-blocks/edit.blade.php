...
<div class="admin-card form-card">
    <form method="POST" action="{{ route('admin.content-blocks.update', $contentBlock) }}">
        ...
        <div class="form-group">
            ...
            <select id="section_id" name="section_id" class="form-select" required>
                @foreach($sections as $section)
                    <option value="{{ $section->id }}" {{ old('section_id', $contentBlock->section_id) == $section->id ? 'selected' : '' }}>{{ $section->page->name }} — {{ $section->name }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input id="key" name="key" type="text" class="form-control" value="{{ old('key', $contentBlock->key) }}" required>
        ...
        <div class="form-group">
            ...
            <select id="type" name="type" class="form-select" required>
                ...
                <option value="link" {{ old('type', $contentBlock->type) == 'link' ? 'selected' : '' }}>Link</option>
                <option value="list" {{ old('type', $contentBlock->type) == 'list' ? 'selected' : '' }}>List</option>
            </select>
            @error('type')<div class="form-text" style="color:#b91c1c;">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
            <label for="content" class="form-label">Content</label>
            <textarea id="content" name="content" class="form-control" rows="4">{{ old('content', $contentBlock->content) }}</textarea>
            @error('content')<div class="form-text" style="color:#b91c1c;">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
            <label for="url" class="form-label">URL</label>
            <input id="url" name="url" type="url" class="form-control" value="{{ old('url', $contentBlock->url) }}">
            @error('url')<div class="form-text" style="color:#b91c1c;">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
            <label for="sort_order" class="form-label">Sort Order</label>
            <input id="sort_order" name="sort_order" type="number" class="form-control" value="{{ old('sort_order', $contentBlock->sort_order) }}">
            @error('sort_order')<div class="form-text" style="color:#b91c1c;">{{ $message }}</div>@enderror
        </div>
        <div class="admin-actions">
            <button type="submit" class="admin-btn">Update Block</button>
