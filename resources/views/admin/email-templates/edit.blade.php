...
<div class="page-form-shell">
    ...
    <form id="et-form" method="POST" action="{{ route('admin.email-templates.update', $emailTemplate) }}" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="page-form-grid et-form-grid">
            ...
            <div class="form-section">
                <div class="form-section-header">
                    ...
                    <div>
                        <div class="form-section-title">Edit Template</div>
                        <div class="form-section-sub">Editing: <strong>{{ $emailTemplate->name }}</strong></div>
                    </div>
                </div>
                ...
                <div class="pf-group {{ $errors->has('name') ? 'has-error' : '' }}">
                    ...
                    <input id="name" name="name" type="text" class="pf-input"
                        ...
                        placeholder="e.g. Welcome Email" autocomplete="off" required>
                    @error('name')
                ...
                <div class="pf-group {{ $errors->has('subject') ? 'has-error' : '' }}">
                    ...
                    <input id="subject" name="subject" type="text" class="pf-input"
                        ...
                        placeholder="e.g. Welcome, @{{name}}! 🎉" autocomplete="off" required>
                    @error('subject')
                        <div class="pf-error">⚠ {{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="pf-group {{ $errors->has('body') ? 'has-error' : '' }}">
                    ...
                    <input type="hidden" name="body" id="body-input" value="{{ old('body', $emailTemplate->body) }}">
                ...
                @php
                    ...
                    if ($emailTemplate->variables) {
                        foreach ($emailTemplate->variables as $k => $v) {
                            $varsText .= $k . ': ' . $v . "\n";
                        }
                    }
                @endphp
                <div class="pf-group {{ $errors->has('variables') ? 'has-error' : '' }}">
                    ...
                    <textarea id="variables" name="variables" class="pf-input pf-textarea" rows="4"
                        placeholder="name: Recipient's full name&#10;email: Recipient's email address&#10;date: Current date&#10;message: Custom message">{{ old('variables', trim($varsText)) }}</textarea>
                    @error('variables')
                        <div class="pf-error">⚠ {{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-sidebar">
                ...
                <div class="form-section">
                    ...
                    <div style="margin-top:16px;padding:12px;background:var(--surface-strong);border-radius:12px;font-size:12px;color:var(--text-muted);line-height:1.8;">
                        <div>Created: {{ $emailTemplate->created_at->format('M j, Y g:i A') }}</div>
                        <div>Updated: {{ $emailTemplate->updated_at->format('M j, Y g:i A') }}</div>
                    </div>
                </div>
                ...
                {{-- Live preview card --}}
                <div class="form-section et-preview-card" id="preview-card">
                    <div class="form-section-header">
                        <span class="form-section-icon">👁️</span>
                        <div>
...
<style>
.et-form-grid { grid-template-columns: 1fr 380px; }
...
.et-var-tip {
    ...
    background: var(--surface-strong); border-radius: 8px; padding: 8px 10px; line-height: 1.5;
}
...
/* ── Live preview (sidebar — simple placeholder swap) ── */
var OB = '{' + '{';
var CB = '}' + '}';
var sampleData = {};
...
function updateLivePreview() {
    livePreview.innerHTML = applyPlaceholders(quill.root.innerHTML);
}
